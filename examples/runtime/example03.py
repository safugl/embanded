...
keys = ['EMB1 (PyTorch)', 'EMB2 (PyTorch)', 'EMB3 (PyTorch)', 'EMB4 (PyTorch)']
...
sweep = [128, 512, 1024, 2048]

df = []
for num_dim in sweep:
    ...
    for key in keys:
        ...
        df.append(dict(num_dim=num_dim,
                  model=key,
                  time_elapsed=time_elapsed_i))

        print('%s, %i, %0.3f' % (key, num_dim, time_elapsed_i))
...
print('Model | ', end=' ')
for num_dim in sweep:
    print('%i dimensions | ' % num_dim, end=' ')
print('')
print(':-|:-|:-|:-|:-|:-')
for key in keys:
    print('%s | ' % key, end=' ')
    for num_dim in sweep:
        time_elapsed, = df.loc[(df.model == key) & (
            df.num_dim == num_dim), 'time_elapsed'].to_numpy()
